...
export function rectHandler(node) {
  let path = void 0;
...
export function circleHandler(node) {
  const cx = node.cx,
    cy = node.cy,
    r = node.r;
  const path =
...
export function ellipseHandler(node) {
  const cx = node.cx,
    cy = node.cy,
    rx = node.rx,
    ry = node.ry;
...
export function lineHandler(node) {
...
export function formateColor(prop) {
  if (!prop) {
    return 'transparent';
  } else if (prop === '#000000') {
    return '';
  ...
  switch (type.toLowerCase()) {
    ...
    case 'path':
      return {
        d: node.d,
        fill:
          node.fill === undefined && node.fill === '#000000' ? '' : node.fill,
        stroke: formateColor(node.stroke),
      };
    case 'polygon':
    case 'polyline':
